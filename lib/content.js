...
export const getResources = cache(async (options = {}) => {
  ...
  return withFallback(async () => {
    ...
    if (options.category && options.category !== "all") {
      ...
      // Videos always have category_slug='video', so exclude them when filtering by article categories
      if (options.category !== "video") {
        videosQuery = videosQuery.limit(0); // Effectively exclude videos
      }
    }
    ...
    const allItems = [
      ...(articlesRes.data ?? []),
      ...(options.type === "video" ? [] : videosRes.data ?? []),
    ]
