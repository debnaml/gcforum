...
import { mockMembers, mockResources, mockEvents } from "./data/mockContent";
...
function normalizeEventResource(resource, index = 0) {
  if (!resource) return null;
  const fileUrl = resource.file_url ?? resource.fileUrl ?? null;
  if (!fileUrl) {
    return null;
  }
  const rawSize = resource.file_size_bytes ?? resource.fileSizeBytes ?? null;
  const fileSizeBytes = typeof rawSize === "number" ? rawSize : rawSize ? Number(rawSize) : null;
  return {
    id: resource.id ?? resource.resource_id ?? `event-resource-${index}`,
    title: resource.title ?? "Download",
    description: resource.description ?? "",
    fileUrl,
    fileType: resource.file_type ?? resource.fileType ?? null,
    fileSizeBytes,
    storagePath: resource.storage_path ?? resource.storagePath ?? null,
    position: typeof resource.position === "number" ? resource.position : index,
...
export function normalizeEvent(row) {
  ...
  const timezone = row.timezone ?? row.time_zone ?? "Europe/London";
  const capacityValue = row.capacity ?? row.max_capacity ?? null;
  const attendeeCountValue = row.attendee_count ?? row.attendees ?? null;
  const keyTakeaways = Array.isArray(row.key_takeaways ?? row.keyTakeaways) ? row.key_takeaways ?? row.keyTakeaways : [];
  const resourcesRaw = Array.isArray(row.resources) ? row.resources : [];
  const resources = resourcesRaw.map((resource, index) => normalizeEventResource(resource, index)).filter(Boolean);
  const occursOn = row.occurs_on ?? row.occursOn ?? (startsAt ? startsAt.slice(0, 10) : null);
  const isPast = row.is_past ?? row.isPast ?? false;
  const isUpcoming = row.is_upcoming ?? row.isUpcoming ?? !isPast;

  if (!row.slug || !startsAt) {
    return null;
  }
  ...
  return {
    ...
    agendaHtml: row.agenda_html ?? row.agendaHtml ?? null,
    heroImageUrl: row.hero_image_url ?? row.heroImageUrl ?? null,
    heroImageAlt: row.hero_image_alt ?? row.heroImageAlt ?? null,
    format: row.format ?? row.category ?? "roundtable",
    focusArea: row.focus_area ?? row.focusArea ?? null,
    locationLabel: row.location_label ?? row.locationLabel ?? row.location_city ?? row.locationCity ?? "",
    locationCity: row.location_city ?? row.locationCity ?? null,
    locationRegion: row.location_region ?? row.locationRegion ?? null,
    ...
    endsAt,
    timezone,
    registrationUrl: row.registration_url ?? row.registrationUrl ?? null,
    registrationLabel: row.registration_label ?? row.registrationLabel ?? "Register",
    ctaLabel: row.cta_label ?? row.ctaLabel ?? null,
    ctaUrl: row.cta_url ?? row.ctaUrl ?? null,
    externalPartner: row.external_partner ?? row.externalPartner ?? null,
    status: row.status ?? "draft",
    featured: Boolean(row.featured),
    capacity: typeof capacityValue === "number" ? capacityValue : capacityValue ? Number(capacityValue) : 0,
    attendeeCount: typeof attendeeCountValue === "number" ? attendeeCountValue : attendeeCountValue ? Number(attendeeCountValue) : 0,
    keyTakeaways,
    seoTitle: row.seo_title ?? row.seoTitle ?? null,
    seoDescription: row.seo_description ?? row.seoDescription ?? null,
    seoImageUrl: row.seo_image_url ?? row.seoImageUrl ?? null,
    ...
    resources,
    resourceCount: typeof row.resource_count === "number"
      ? row.resource_count
...
function createMockEventRow(event = {}, overrides = {}) {
  const iso = event.date ? new Date(event.date).toISOString() : new Date().toISOString();
  return {
    id: event.id ?? overrides.id ?? `mock-event-${Math.random().toString(36).slice(2)}`,
    slug: event.slug ?? event.id ?? overrides.slug ?? `mock-event-${Math.random().toString(36).slice(2)}`,
    title: event.title ?? overrides.title ?? "GC Forum session",
    summary: event.summary ?? overrides.summary ?? event.category ?? "",
    description_html: event.description_html ?? overrides.description_html ?? "",
    format: event.format ?? overrides.format ?? event.category ?? "session",
    location_label: event.location_label ?? overrides.location_label ?? event.location ?? "London",
    location_city: event.location_city ?? overrides.location_city ?? event.location ?? "London",
    location_region: event.location_region ?? overrides.location_region ?? null,
    location_country: event.location_country ?? overrides.location_country ?? "UK",
    venue_name: event.venue_name ?? overrides.venue_name ?? null,
    starts_at: overrides.starts_at ?? iso,
    ends_at: overrides.ends_at ?? iso,
    timezone: overrides.timezone ?? "Europe/London",
    registration_label: event.cta ?? overrides.registration_label ?? "Register",
    cta_label: event.cta ?? overrides.cta_label ?? event.cta ?? null,
    capacity: event.capacity ?? overrides.capacity ?? 0,
    attendee_count: event.attendees ?? overrides.attendee_count ?? 0,
    status: "published",
    featured: false,
    resources: [],
    key_takeaways: [],
    is_past: overrides.is_past ?? false,
    is_upcoming: overrides.is_upcoming ?? true,
  };
}

function buildMockEventsFallback() {
  const upcoming = Array.isArray(mockEvents?.upcoming)
    ? mockEvents.upcoming.map((event) => normalizeEvent(createMockEventRow(event, { is_past: false, is_upcoming: true }))).filter(Boolean)
    : [];
  const past = Array.isArray(mockEvents?.past)
    ? mockEvents.past.map((event) => normalizeEvent(createMockEventRow(event, { is_past: true, is_upcoming: false }))).filter(Boolean)
    : [];
  return { upcoming, past };
}

const MOCK_EVENTS_FALLBACK = buildMockEventsFallback();

function findMockEventBySlug(slug) {
  if (!slug) {
    return null;
  }
  const safeSlug = slug.toString();
  const catalog = [...MOCK_EVENTS_FALLBACK.upcoming, ...MOCK_EVENTS_FALLBACK.past];
  return catalog.find((event) => event.slug === safeSlug) ?? null;
}
...
export const getEvents = cache(async () => {
  ...
  return withFallback(async () => {
    ...
    const normalized = (data ?? []).map((row) => normalizeEvent(row)).filter(Boolean);
    const upcoming = normalized
      .filter((event) => event.isUpcoming)
      .sort((a, b) => new Date(a.startsAt || 0) - new Date(b.startsAt || 0));
    const past = normalized
      .filter((event) => event.isPast && !event.isUpcoming)
      .sort((a, b) => new Date(b.startsAt || 0) - new Date(a.startsAt || 0));

    return { upcoming, past };
  }, MOCK_EVENTS_FALLBACK);
});
...
export const getEventBySlug = cache(async (slug) => {
  ...
  const supabase = await getServerClient();
  if (!supabase) {
    return findMockEventBySlug(slug);
  }
